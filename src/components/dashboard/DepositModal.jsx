...
import { makeDeposit } from "../../api/deposit";

export default function DepositModal({ open, onClose, userId, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await makeDeposit({ userId, plan, amount: Number(amount), method, token });
      setLoading(false);
      onSuccess?.();
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
      <form onSubmit={handleSubmit} className="bg-[#181a20] p-6 rounded-xl w-80 flex flex-col gap-4 shadow-lg">
        <div className="text-lg font-bold text-[#bfa233] mb-2">Deposit Funds</div>
        ...
        <label className="text-xs text-white">Plan
          <select value={plan} onChange={e => setPlan(e.target.value)} className="w-full mt-1 p-2 rounded bg-[#23272f] text-white">
        ...
        <div className="flex gap-2 mt-2">
          ...
          <button type="submit" disabled={loading} className="flex-1 py-2 rounded bg-[#bfa233] text-black font-bold disabled:opacity-60">{loading ? "Depositing..." : "Deposit"}</button>
        </div>
      </form>
    </div>
  );
